...
#define MAKE_1TO1_PTR(p)	((void *)((paddr32_t)(p) + shdr->paddr_bias))
#endif
...
#define COMMON_OPTIONS_STRING   CPU_COMMON_OPTIONS_STRING "ACD:F:f:I:i:K:M:N:P:R:S:Tvr:j:ZH"
...
uintptr_t	callout_io_map32(unsigned size, paddr32_t phys);
uintptr_t	callout_io_map64(unsigned size, paddr64_t phys);

void		*callout_memory_map(unsigned size, paddr_t phys, unsigned prot_flags);
void		*callout_memory_map32(unsigned size, paddr32_t phys, unsigned prot_flags);
void		*callout_memory_map64(unsigned size, paddr64_t phys, unsigned prot_flags);
...
void                        init_pl011(unsigned channel, const char *init, const char *defaults);
void 						put_pl011(int);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/startup/lib/startup.h $ $Rev: 796209 $")
#endif
