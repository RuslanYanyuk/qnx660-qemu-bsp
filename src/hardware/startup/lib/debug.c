...
print_syspage(void) {
	...
	if(debug_flag > 1) {
		...
		while((opt = getopt(_argc, _argv, "S:")) != -1) {
			if(opt == 'S') {
                kprintf("printing syspage for section");
				print_syspage_enable(optarg);
			}
		}
        kprintf("printing syspage");
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/startup/lib/debug.c $ $Rev: 680332 $")
#endif
